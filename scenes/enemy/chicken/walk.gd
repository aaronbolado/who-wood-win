...
func _ready() -> void:
	...
	# call_deferred
	# useful method for calling other functions after current frame is finished processing
	# will call character_setup during idle time
	# when character_setup called, it will wait for first physics_frame
...
func _on_physics_process(_delta : float) -> void:
	...
	if navigation_agent_2d.avoidance_enabled:
		...
		# not using delta bcz character uses delta time in move_and_slide
		# only need to calc the direction and speed
